...
module.directive('configSet', function() {
    return {
    	restrict: 'EA',
    	templateUrl: 'realtimeReceive/partials/config-set.html',
        replace: true,
        controller: function($scope,$element,$compile, cleancompConfig) {
           ...
           $scope.generateField=function(agg,index){
                if (agg.indexOf("Regex")>=0) {
                    agg=agg.split(",'");
                    console.log(agg[agg.length-1].replace("')",""))
                    $scope.resultcount=agg[agg.length-1].replace("')","").split(",").length
                    console.log($scope.resultcount)
                    $scope.generate(agg[agg.length-1].replace("')","").split(","),$scope.resultcount,index)
                };
                /* agg="Regex(A, '(.*),(.*),(.*)','1,2,3,4')"*/
           ...
           $scope.generate=function(agg,num,index){
               var leng=cleancompConfig.config.config.filterDetail[index].resultArr.length 
               if (num>leng) {
                 var difference=num-leng
                 for (var i = 0; i < difference; i++) {
                     var lengIndex=leng+i
                     var addres={
                        resultName:'result'+index+agg[i],
                        type:'string',
                        isTime:false,
                        dateFormat:''
                     }
                     cleancompConfig.config.config.filterDetail[index].resultArr.push(addres)
                 }
               }else if(num<leng){
                 var difference=leng-num
                 console.log("index==="+index)
                 console.log(difference)
                 for (var i = 0; i < difference; i++) {
                     var lengIndex=leng+i+1
                     cleancompConfig.config.config.filterDetail[index].resultArr.splice(num,leng-num)
                 };
