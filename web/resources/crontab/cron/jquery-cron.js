...
(function($) {
    ...
    var periodsText = ["分", "时", "天", "周", "月", "年"];
    for (var i = 0; i < periods.length; i++) {
        str_opt_period += "<option value='"+periods[i]+"'>" + periodsText[i] + "</option>\n";
    }
    ...
    var combinations = {
        ...
        "month"  : /^(\d{1,2}\s){2}((\d{1,2}|\*),*)+\s\*\s\*$/,         // "? ? ? * *"
        "year"   : /^(((\d{1,2}|\*),*)+\s){4}((\d{1,2}|\*),*)+$/            // "? ? ? ? *"
    };
    ...
    var methods = {
        init : function(opts) {
            ...
            block["month"] = $("<span class='cron-block cron-block-month'>"
                    + " 的 <select name='cron-month' multiple=true >" + str_opt_month
                    + "</select> </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["dom"] = $("<span class='cron-block cron-block-dom'>"
                    + " 的 <select name='cron-dom' multiple=true >" + str_opt_dom
                    + "</select> </span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["mins"] = $("<span class='cron-block cron-block-mins'>"
                    + " 过 <select name='cron-mins'>" + str_opt_mih
                    + "</select></span>")
                .appendTo(this)
                .data("root", this);
            ...
            block["dow"] = $("<span class='cron-block cron-block-dow'>"
                    + " 的 <select name='cron-dow' multiple=true>" + str_opt_dow
                    + "</select> </span>")
                .appendTo(this)
                .data("root", this);
