	...
	$.fn.mapael = function(options) {
		...
		return this.each(function() {
			...
			var $self = $(this)
				...
				, paper = new Raphael($container[0], options.map.width, options.map.height)
				, elemOptions = {}
				, resizeTO = 0
				, areas = {}
				, plots = {}
				, legends = []
				, ratio = options.map.ratio
				, id = 0;
			...
			for (id in mapConf.elems) {
				...
				if (ratio!=1) {
					...
					oldmappath = mapConf.elems[id];
					oldmappath = oldmappath.replace('M','').replace('Z', '');
					var oldpoints = oldmappath.split('L');
					var points = [], tmps, newX, newY;
					$.each(oldpoints, function(pi,pv){
						tmps = pv.split(' ');
						newX = parseFloat(tmps[0])*ratio;
						newY = parseFloat(tmps[1])*ratio;
						points.push(newX+' '+newY);
					});
					
					mappath = 'M'+(points.join('L'))+'Z';
				} else {
					mappath = mapConf.elems[id];
				}
				areas[id] = {"mapElem" : paper.path(mappath).attr(elemOptions.attrs)};
			}
	...
	*/
	$.fn.mapael.setTooltip = function(elem, $tooltip) {
		var tooltipTO = 0
			...
			, containerY2 = $container.offset().left ,containerY3 = $container.offset().top;//containerY2 = $container.offset().left + $container.width();
		
		$(elem.node).on("mouseover", function(e) {
			tooltipTO = setTimeout(
				function() {
					elem.tooltipContent && $tooltip.html(elem.tooltipContent).css("display", "block");
					$tooltip.css({"left" : Math.min(containerY2 - $tooltip.outerWidth() - 5, e.pageX + 10 - $(window).scrollLeft()), "top" : e.pageY + 20 - containerY3});
				}
				, 120
			);
