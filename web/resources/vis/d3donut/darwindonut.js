...
*/
var DarwinDonut = function(domid, data) {
	this.domid = domid;
	this.data = data;
	this.finalOptions = {};
	 ...
	 */
	DarwinDonut.prototype.draw = function(width, height, options) {
		...
		if(typeof window['_darwin_is_body_clk_donut_tt_binded'] == "undefined") {
			$('body').click(function(e){
				var segmentClick = window['_darwin_donut_segment_clk_label'];
	            if (typeof segmentClick!="undefined" 
	            	&& segmentClick!="") {
	            	$('._darwin_tooltip').css('top', e.pageY - 20).css('left', e.pageX).text(segmentClick).show();
	            }
	        });
			window['_darwin_is_body_clk_donut_tt_binded'] = 1;
		}
		
		this.finalOptions['callbacks'] = {
	        	'onMouseoutSegment':function(info) {
	        		window['_darwin_donut_segment_clk_label'] = '';
	                $('._darwin_tooltip').text('').hide();
	            },
	            'onClickSegment':function(info) {
	            	window['_darwin_donut_segment_clk_label'] = info.data.label;
	            }
	        };
	...
	//参数解析
	DarwinDonut.prototype.parseOptions = function(options) {
	...
	//递归设置属性值
	DarwinDonut.prototype._rescureSetJson = function(dict, keylist, val) {
	 ...
	 * 为每个数据计算其颜色
	 */
	DarwinDonut.prototype.makeColorSerial = function(data) {
